...
public class AutonomousStatusWebsocket implements Observer{
    ...
    public void update(Observable o, Object arg) {
        try {
            ...
            if (status.getSnapshot() != null) {
                MatOfByte ofByte = new MatOfByte();
                Imgcodecs.imencode(".png", status.getSnapshot(), ofByte);
                encoded = new String(Base64.encode(ofByte.toArray()));
            }
            String json = mapper.writeValueAsString(status);
            for (Session session : sessions) {
                session.getAsyncRemote().sendText(json);
                if (encoded != null) {
                    session.getAsyncRemote().sendText(encoded);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
