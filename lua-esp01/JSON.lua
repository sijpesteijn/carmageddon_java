local VERSION = 20160709.16 -- version history at end of file
local AUTHOR_NOTE = "-[ JSON.lua package by Jeffrey Friedl (http://regex.info/blog/lua/json) version 20160709.16 ]-"
local OBJDEF = {
   VERSION      = VERSION,
...
function OBJDEF:asNumber(item)
   ...
   elseif type(item) == 'table' and type(item.S) == 'string' and type(item.N) == 'number' then
      -- it's a number-object table that lots its metatable, so give it one
